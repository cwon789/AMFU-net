...
def save_train_log(args, save_dir):
    ...
    with open('./AMFU-net/result/%s/train_log.txt'%save_dir ,'w') as  f:
        now = datetime.now()
        f.write("time:--")
...
def save_model(mean_IOU, best_iou, save_dir, save_prefix, train_loss, test_loss, recall, precision, epoch, net):
    if mean_IOU > best_iou:
        save_mIoU_dir = './AMFU-net/result/' + save_dir + '/' + save_prefix + '_best_IoU_IoU.log'
        save_other_metric_dir = './AMFU-net/result/' + save_dir + '/' + save_prefix + '_best_IoU_other_metric.log'
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        best_iou = mean_IOU
...
def make_dir(deep_supervision, dataset, model):
    ...
    os.makedirs('./AMFU-net/result/%s' % save_dir, exist_ok=True)            # Set save directory
    return save_dir
